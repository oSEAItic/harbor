...
func (s *Store) BindSession(ctx context.Context, binding SessionBinding) error {
	if strings.TrimSpace(binding.FeatureID) == "" || strings.TrimSpace(binding.HarborSessionID) == "" {
	...
	defer tx.Rollback()
	if _, err = tx.ExecContext(ctx, `INSERT OR IGNORE INTO feature_sessions(feature_id, harbor_session_id, source, external_session_id, repo_path, branch, bound_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		binding.FeatureID, binding.HarborSessionID, binding.Source, binding.ExternalSessionID, binding.RepoPath, binding.Branch, formatTime(binding.BoundAt)); err != nil {
...
func (s *Store) Detail(ctx context.Context, id string) (Detail, error) {
	...
	rows.Close()
	rows, err = s.db.QueryContext(ctx, `SELECT feature_id, harbor_session_id, source, external_session_id, repo_path, branch, bound_at FROM feature_sessions WHERE feature_id = ? ORDER BY bound_at`, id)
	if err != nil {
		return Detail{}, err
	}
	for rows.Next() {
		...
		var at string
		if err := rows.Scan(&b.FeatureID, &b.HarborSessionID, &b.Source, &b.ExternalSessionID, &b.RepoPath, &b.Branch, &at); err != nil {
			rows.Close()
			return Detail{}, err
		}
